...
<?php
    ...
    $link=mysqli_connect($db['db_host'], $db['db_user'], $db['db_password'], $db['db_name']);

    $section_id = $_GET['site'] == "programista" ? '1' : '2';
    if(isset($_POST['delete'])){
        //$link -> query('DELETE FROM artykuły WHERE id ='.$_POST['delete'].'');
    }

    if(isset($_POST['delete'])){
      $user = $link->query("select * from ".$db['db_prefix']."użytkownik where id = ".$_SESSION['userid'])->fetch_assoc();
      if($user['admin'] || ($user['programista_art'] && $section_id == 1) || $user['gastronomia_art'] && $section_id == 2){
        $query = 'delete from '.$db['db_prefix'].'artykuły where id = ?';
    ...
    $query = "select * from ".$db['db_prefix']."artykuły where sekcja ='".$section_id."' ORDER BY `id` DESC;";
    $result = $link->query($query);

    if($result->num_rows==0) echo '<p>Brak postów do wyświetlenia</p>';
    

    if(isset($_SESSION['userid'])) $adminPerms = $link->query('select admin from '.$db['db_prefix'].'użytkownik where id = '.$_SESSION['userid'].';')->fetch_assoc()['admin'];
    ...
    if($result){
      while($wiersz=$result->fetch_assoc()){
          ...
          if(!$wiersz['autor_anon']){
            $author = $link->query('select imie, nazwisko from '.$db['db_prefix'].'użytkownik where id = '.$wiersz['autor'].';')->fetch_assoc();
